...
class Item(Resource):  # every resource is a class.  Stuedent thee=reofre inherits Cla  Resource
    ...
    def post(self, name):
        ...
        item = ItemModel(name, **data) # **data is same as data['price'], data['store_id]'])       # price keywd of data dictionary - ``#to python convert # json payload and correct Content Type header (Appln / jsom) is required to enalbe request.get_json() to work
